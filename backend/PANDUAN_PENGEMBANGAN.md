...
* **Database:** SQLite3 terintegrasi secara lokal, menggunakan library `sqlite` dan `sqlite3`. Data disimpan di file lokal.
...
3. **Backend (`src/ipc/`)**: Menerima request lewat `ipcMain.handle`, memproses query ke database SQLite, dan mengembalikan data dalam format JSON/Object.
...
* **File Kuitansi / PDF:** Menggunakan library `jspdf` dari CDN di sisi frontend. Semua struk dan laporan digenerate murni di client-side untuk kecepatan.
* **Database Persistensi:** Karena ini SQLite lokal, data disimpan dalam satu file `.db`. Jika Anda ingin mem-backup data sekolah, cukup backup file `.db` tersebut (ditempatkan di folder khusus system).
