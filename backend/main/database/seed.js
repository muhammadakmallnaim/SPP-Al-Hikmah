...
async function seedData() {
    ...
    await db.run('INSERT INTO kelas (nama_kelas, tingkat, wali_kelas) VALUES (?, ?, ?)', ['X-A', 10, 'Budi Santoso']);
    await db.run('INSERT INTO kelas (nama_kelas, tingkat, wali_kelas) VALUES (?, ?, ?)', ['XI-A', 11, 'Siti Aminah']);
    await db.run('INSERT INTO kelas (nama_kelas, tingkat, wali_kelas) VALUES (?, ?, ?)', ['XII-A', 12, 'Ahmad Dahlan']);
    ...
    await db.run(querySiswa, ['2025002', '0012345672', 'Budi Santoso', 'L', 'Gebang', '2007-06-15', 'Jl. Pahlawan No 2', 'Bapak Santoso', '081234567891', 2, 2024, 'aktif', '2025002']);
    await db.run(querySiswa, ['2025003', '0012345673', 'Citra Kartini', 'P', 'Tanjung Pura', '2006-07-20', 'Jl. Merdeka No 3', 'Bapak Kartini', '081234567892', 3, 2023, 'aktif', '2025003']);
    ...
    await db.run(querySPP, [1, 1, 75000, 5]); // Kelas X
    await db.run(querySPP, [1, 2, 85000, 5]); // Kelas XI
    await db.run(querySPP, [1, 3, 90000, 5]); // Kelas XII
    ...
    await db.run(queryRiwayat, [2, 2, 1]);
    await db.run(queryRiwayat, [3, 3, 1]);
